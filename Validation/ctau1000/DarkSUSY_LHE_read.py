import ROOT, array, os, re, math, random, string, sys
from math import *
...
#filename = "DarkSUSY_mH_125_mGammaD_0250_13TeV_cT_000_madgraph452_bridge224_events80k.lhe"
#filenamefull = "/home/taohuang/MuJetAnalysis_DarkSusySamples_LHE_Generation/MG_ME_V4.5.2_CompiledBackup/MG_ME_V4.5.2/BRIDGE/DarkSUSY_mH_125_mGammaD_0400_13TeV-madgraph452_bridge224_events10k.lhe"
filenamefull = sys.argv[1]
...
for i in infolist:
	if i=="mH":
		mass_Higgs = infolist[nlist+1]
	if i=="mGammaD":
		mass_GammaD = infolist[nlist+1]
		mass_GammaD_Legend = infolist[nlist+1]
	if i=="cT":
		lifetime_GammaD = infolist[nlist+1]
		lifetime_GammaD_Legend = infolist[nlist+1]
	if i[-3:]=="TeV":
		energy = infolist[nlist]
	nlist +=1
"""
if len(filename) == 77:
...
"""
print "energy ", energy
print "mass_GammaD ",mass_GammaD
print "lifetime_GammaD ",lifetime_GammaD
print "lifetime_GammaD_Legend ", lifetime_GammaD_Legend
print "mass_GammaD_Legend ",mass_GammaD_Legend
...
info.SetMargin(0.13)
info.SetHeader("#splitline{pp #rightarrow h #rightarrow 2n_{1} #rightarrow 2n_{D} + 2 #gamma_{D} #rightarrow 2n_{D} + 4#mu}{#splitline{m_{h} = " + mass_Higgs +  " GeV, m_{n_{1}} = 50 GeV, m_{n_{D}} = 1 GeV}{m_{#gamma_{D}} = " + mass_GammaD_Legend + " GeV, c#tau_{#gamma_{D}} = " + lifetime_GammaD_Legend + " mm}}" )
...
if float(lifetime_GammaD_Legend) != 0:
	...
	binwidth = float(lifetime_GammaD_Legend)/10.0
...
h_m2 = ROOT.TH1F("h_m2", "h_m2", 101, 10.1, 30.1)
...
#		  print "P", line
		  ...
		  if abs(IDUP) == higgsID:
		    ...
		    print "higgs lorentz vector ",vHiggs.Print()
		    print "higgs Mass ",vHiggs.M()," filled value ",higgs[len(higgs)-1][5]
